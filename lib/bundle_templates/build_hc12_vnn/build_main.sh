...
if true; then
	...
	# insert main content archive and corresponding command script
	# for files which are independend from the running BTHFCK2 version
	rm -R -f ./arv$n
	mkdir arv$n
	mv ./command.sh 	./arv$n
	mv ./content.tar	./arv$n
	tar cvfz  arv$n.tgz arv$n
	rm -R -f ./arv$n
	echo v$1:v$2=command/arv$n.tgz >> compat.txt
	(( n += 1 )) # archive number increment
...
echo "    fi" >> command.sh
echo >> command.sh
echo "    exec_compfile	command/compat.txt" >> command.sh
echo >> command.sh
